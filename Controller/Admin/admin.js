...
const IMAGE_ID = require('../../Model/ImageID');
...
ROUTER.use(REQUIRED_LOGIN_STAFF);
ROUTER.get('/customer_request_list', ASYNC_HANDLER(async function getUserRequestVerify(req, res) {

    const CUSTOM_VERRIFY_ACCOUNT = await VERRIFY_ACCOUNT.findAllRequestNotCheck();
    console.log( CUSTOM_VERRIFY_ACCOUNT)
    const IMAGE = await IMAGE_ID.findAllImage();   
    console.log(IMAGE)
    res.render('Customer_Request_List', { moment: MOMENT, verifyAccount: CUSTOM_VERRIFY_ACCOUNT, img: IMAGE})
}));

ROUTER.get('/:id/requestVerify', ASYNC_HANDLER(async function RequestDone(req, res) {
    const { id } = req.params;
    const CUSTOM_VERRIFY_ACCOUNT = await VERRIFY_ACCOUNT.updateRequest_False(id);
    res.redirect('/admin/customer_request_list');
}));
